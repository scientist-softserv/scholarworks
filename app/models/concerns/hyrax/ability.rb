...
module Hyrax
  module Ability
    ...
    def admin_set_with_deposit?
      ...
      ### CUSTOMIZATION
      # need to send as post query
      # was: ActiveFedora::SolrService.count(query).positive?

      args = { rows: 0 }
      ActiveFedora::SolrService.post(query, args)['response']['numFound'].to_i.positive?
    ...
    def user_is_depositor?(document_id)
      ...
      ### CUSTOMIZATION
      # add rows, otherwise we get warnings

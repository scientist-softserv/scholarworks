    ...
    class CreatorAttributeRenderer < FacetedAttributeRenderer 
      ...
      def render
        ...
        Array(values).each_with_index do |value, index|
          markup << "<li#{html_attributes(attributes)}>#{attribute_value_to_html(value.to_s, extra_values[0][index], extra_values[1][index])}</li>"
        end
        markup << %(</ul></td></tr>)
        markup.html_safe
      end
      ...
      private
        ...
        def attribute_value_to_html(value1, value2, value3)

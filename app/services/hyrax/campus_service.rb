module Hyrax
  ...
  module CampusService
    ...
    #
    # Get campus name from slug
    #
    # @param [String] campus_slug short campus code, e.g., 'losangeles'
    #
    # @return [String] the campus name, e.g., 'Los Angeles'
    #
    def self.get_campus_name_from_slug(campus_slug)
      CAMPUSES.select do |campus|
        campus[:slug].downcase == campus_slug.downcase
      end.first&.fetch(:name)
    end
